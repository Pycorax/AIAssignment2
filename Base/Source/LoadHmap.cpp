...
float ReadHeightMap(std::vector<unsigned char> &heightMap, float x, float z)
{
	...
	unsigned mapLength = static_cast<unsigned>(sqrt(static_cast<double>(heightMap.size())));

	unsigned xCoord = static_cast<unsigned>((x + 0.5f) * mapLength);
	unsigned zCoord = static_cast<unsigned>((z + 0.5f) * mapLength);

	return static_cast<float>(heightMap[zCoord * mapLength + xCoord] / 256.0f);
}